        ...
        if ($request->ajax()) {
            return datatables(Student::with('center:id,code','subject:id,name','result'))
                ...
                ->editColumn('certificate', function ($certificate) {
                    return '<a   style="background-color:green; padding:3px; border-redius:4px 4px 4px 4px; color:white"   target="_blank" href="' . route("admin.student.studentCertificate",[$certificate->id,'certificate'=>'certificate']) . '">' . "Certificate" . '</a>';
                })
