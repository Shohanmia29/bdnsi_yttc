<x-app-layout>
    ...
    <form action="{{ route('student.update', $student->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-wrap justify-center w-full bg-white p-4">
            ...
            <x-labeled-input label="Mobile No" name="phone" :value="$student->phone" pattern="\d{11}" x-data x-on:input="$event.target.setCustomValidity($event.target.validity.patternMismatch ? 'Phone number should be 11 digits' : '')" required class="w-full p-1 md:w-1/2 lg:w-1/3"/>
