        ...
        <div class="w-full    bg-white  overflow-x-auto  ">
            <div class="">
                <div class="r-bg  min-h-[100vh] print:min-h-[50vh] flex-col justify-center   w-full relative text-[16px] font-bold text-gray-500" style="background-image: url({{asset('images/admit.png')}});font-family: 'Times New Roman'">
                    ...
                    <div class="absolute top-[12.3%]  left-[37.5%] print:left-[37.5%] text-[#1C77B9] text-sm py-1 uppercase  tracking-[3px]     w-38  tracking-y-[20px]   ">{{$student->subject->name  .'-'.   date('y',strtotime($student->session->start_date))??''}}</div>
                    <div class="absolute top-[23.3%]   print:top-[22%]  left-[12.5%] w-38  ">{{$student->id??''}}</div>
                    <div class="absolute   top-[19%] print:top-[18%] left-[56.5%] w-38  ">C-{{$student->center->code??''}}/{{date('y',strtotime($student->session->start_date))}} </div>
                    ...
                    <div class="absolute top-[85.5%] text-[#7CA299] print:left-[4%]  left-[7%] w-38 capitalize tracking-[.1em] ">{{$student->center->name??''}}</div>
                </div>
